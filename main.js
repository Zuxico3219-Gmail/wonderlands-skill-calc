...
async function supportsEncode() {
	...
	const avifData = "data:image/avif;base64,",
		webpData = "data:image/webp;baseactionSkill64,UklGRhwAAABXRUJQVlA4TBAAAAAvAAAAEAfQpv5HmQMR0f8A",
...
function updateStats() {
	...
	$(".skill").each(function() {
		...
		if (p > 0) {
			...
			let description = $(this).children(".description").html().replace("<h2>", "<strong>").replace("</h2>", " " + p + ": ").replace("<h3>", "<br>").replace("</h3>", "</strong>").replace(/\n|\r|\t/g, "");
			$(description).each(function(index, element) {
				if (index == 1) {
